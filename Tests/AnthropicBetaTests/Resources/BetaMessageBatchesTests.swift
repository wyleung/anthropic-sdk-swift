...
final class BetaMessageBatchesTests: XCTestCase {
    ...
    func testCreateSendsBodyAndBetaHeader() async throws {
        ...
        let batch = try await client.beta.messages.batches.create(
            requests: [
                .init(
                    customId: "req-1",
                    params: BetaMessageCreateParams(model: "claude-opus-5", maxTokens: 256, messages: [.user("Hi")])
                )
            ],
        ...
        XCTAssertEqual(request.value(forHTTPHeaderField: "anthropic-beta"), "message-batches-2024-09-24")
    ...
    func testRetrieveDecodesBatch() async throws {
        ...
        let client = AnthropicClient(apiKey: "test-key", urlSession: MockURLProtocol.makeSession())
        let batch = try await client.beta.messages.batches.retrieve("msgbatch_01ABC")
    ...
    func testListDecodesPage() async throws {
        ...
        let client = AnthropicClient(apiKey: "test-key", urlSession: MockURLProtocol.makeSession())
        let page = try await client.beta.messages.batches.list(limit: 20)
    ...
    func testDeleteDecodesDeletedBatch() async throws {
        ...
        let client = AnthropicClient(apiKey: "test-key", urlSession: MockURLProtocol.makeSession())
        let deleted = try await client.beta.messages.batches.delete("msgbatch_01ABC")
    ...
    func testCancelPostsToCancelPath() async throws {
        ...
        let client = AnthropicClient(apiKey: "test-key", urlSession: MockURLProtocol.makeSession())
        let batch = try await client.beta.messages.batches.cancel("msgbatch_01ABC")
    ...
    func testResultsSplitsJSONLIntoTypedResults() async throws {
        ...
        let client = AnthropicClient(apiKey: "test-key", urlSession: MockURLProtocol.makeSession())
        let results = try await client.beta.messages.batches.results("msgbatch_01ABC")
    ...
    func testResultsThrowsWhenNoResultsUrl() async throws {
        ...
        do {
            _ = try await client.beta.messages.batches.results("msgbatch_01ABC")
